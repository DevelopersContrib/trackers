  ...
  class Datatables
  {
    ...
    */
	...
	/*
		fulltext search
	*/
	public function match($key_condition, $val = NULL, $backtick_protect = TRUE)
    {
		//$this->db->where('MATCH (field) AGAINST ("value")', NULL, FALSE);
      //$this->where[] = array($key_condition, $val, $backtick_protect);
      $this->ci->db->where('MATCH ('.$key_condition.') AGAINST ("'.$val.'")', NULL, FALSE);
      return $this;
    }
	...
	*/
	 ...
	 public function where_in($key_condition, $valArr = NULL, $backtick_protect = TRUE)
    {
      $this->ci->db->where_in($key_condition, $valArr, $backtick_protect);
      return $this;
    }
    
    
   public function where_not_in($key_condition, $valArr = NULL, $backtick_protect = TRUE)
    {
      $this->ci->db->where_not_in($key_condition, $valArr, $backtick_protect);
      return $this;
    }
    ...
    */
    protected function get_ordering()
    {
	  ...
	  elseif ($this->ci->input->get('sColumns') && preg_match("/[a-z]/i", $this->ci->input->get('sColumns')) )
    ...
    */
    protected function get_filtering()
    {
	  ...
	  //$debug = $this->ci->db->escape($this->ci->input->get('debug'));
	  //if(!empty($debug)){
		//die($sWhere);
	  //}
     ...
     //passed parameters
	  ...
	  $not_in_team_id = $this->ci->db->escape($this->ci->input->get('not_in_team_id'));
	  if ($not_in_team_id!=""){
      	if ($sWhere != ''){
      		$sWhere = " ($sWhere) AND member_id NOT IN (Select member_id from team_member where team_id=$not_in_team_id)";
      	}else {
      		$sWhere = " ($sWhere) member_id NOT IN (Select member_id from team_member where team_id=$not_in_team_id)";
      	}
      }
