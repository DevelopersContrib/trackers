<?php
class LeadsDomainData extends CI_Model {
	private $table = "tracker_leads_domains";
	private $pk = "id";
	private $date = null;
	...
	public function getinfo($find,$field,$value,$field2=null,$value2=null){
	      $v = "";
	      if ($field2 != null){
	      	$query = $this->db->query("SELECT `$find` as val FROM `$this->table` WHERE `$field` = '".$value."' AND `$field2` = '".$value2."'");
	      }else {
	      	$query = $this->db->query("SELECT `$find` as val FROM `$this->table` WHERE `$field` = '".$value."' ");
	      }
	      if ($query->num_rows() > 0){
	        foreach ($query->result() as $row)
	         {
